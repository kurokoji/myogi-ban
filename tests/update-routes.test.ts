...
function fetchGithubAndInstallerWithObsPlugin(
  tagName: string,
  content = "fake exe",
) {
  return async (input: string | URL) => {
...
test("POST /api/update/obs-plugin/download rejects when the environment does not support installing", async () => {
  const manager = new UpdateManager({
    currentVersion: "1.0.17",
  ...
  try {
    const response = await fetch(
      `${server.baseUrl}/api/update/obs-plugin/download`,
      { method: "POST" },
    );
    assert.equal(response.status, 400);
    assert.deepEqual(await response.json(), {
...
test("POST /api/update/obs-plugin/download rejects when there is no OBS plugin asset available", async () => {
  const downloadDirectory = mkdtempSync(
    join(tmpdir(), "myogi-ban-update-routes-"),
  );
  const manager = new UpdateManager({
  ...
  try {
    ...
    assert.deepEqual(await response.json(), {
      ok: false,
      error: "obs_plugin_not_available",
    });
  } finally {
    await server.close();
    rmSync(downloadDirectory, { recursive: true, force: true });
...
test("POST /api/update/obs-plugin/download starts the download, visible via status polling", async () => {
  const downloadDirectory = mkdtempSync(
    join(tmpdir(), "myogi-ban-update-routes-"),
  );
  const manager = new UpdateManager({
  ...
  try {
    await server.postJson("/api/update/obs-plugin/download", {});

    let status = await server.getJson<{
      obsPluginDownload: { state: string };
    }>("/api/update/status");
    for (let i = 0; i < 50 && status.obsPluginDownload.state === "idle"; i++) {
      await new Promise((resolve) => setTimeout(resolve, 10));
      status = await server.getJson("/api/update/status");
    }
    assert.notEqual(status.obsPluginDownload.state, "idle");
  } finally {
    await server.close();
    rmSync(downloadDirectory, { recursive: true, force: true });
  }
...
test("POST /api/update/obs-plugin/install rejects when the installer is not downloaded yet", async () => {
  const downloadDirectory = mkdtempSync(
    join(tmpdir(), "myogi-ban-update-routes-"),
  );
  const manager = new UpdateManager({
  ...
  try {
    const response = await fetch(
      `${server.baseUrl}/api/update/obs-plugin/install`,
      { method: "POST" },
    );
    assert.equal(response.status, 400);
    assert.deepEqual(await response.json(), {
...
test("POST /api/update/obs-plugin/install launches the downloaded OBS plugin installer", async () => {
  const downloadDirectory = mkdtempSync(
    join(tmpdir(), "myogi-ban-update-routes-"),
  );
  let launched: string | undefined;
  ...
  try {
    ...
    for (
      let i = 0;
      i < 50 && status.obsPluginDownload.state !== "downloaded";
      i++
    ) {
      await new Promise((resolve) => setTimeout(resolve, 10));
      status = await server.getJson("/api/update/status");
    ...
    await server.postJson("/api/update/obs-plugin/install", {});
